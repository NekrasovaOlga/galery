...
import likeImg from '../../ImageInfo/img/like.png';
import { likeAuthRequest } from '../../../../store/like/likeAction';
import { useDispatch } from 'react-redux';
import { galeryRequestAsync } from '../../../../store/galery/galeryAction';

export const Images = ({ data, link }) => {
  let { id, created_at: date, urls, user, likes, liked_by_user } = data;
  const [like, setLike] = useState(liked_by_user);
  return (
    <div className={style.container}>
      <Link className={style.linkPost} to={`${link}/${id}`}>
        <img className={style.img} src={urls.small} alt="Картинка" />
      </Link>
      <div className={style.info}>
        ...
        <div className={style.info__user}>
          <p className={style.userName}>Автор: {user.name}</p>
          <p className={style.dataCreated}>Дата создания: {fromDate(date)}</p>
        </div>
      </div>
    </div>
  );
